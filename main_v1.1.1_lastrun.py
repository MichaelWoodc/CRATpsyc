...
expName = 'main_v3'  # from the Builder filename that created this script
expVersion = ''
...
def setupData(expInfo, dataDir=None):
    ...
    thisExp = data.ExperimentHandler(
        ...
        originPath="C:\\Users\\Michael\\OneDrive - Georgia Southern University\\Virginia Wickline's files - crat\\1_main_v1.1.1\\Main_v1.1.1\\main_v1.1.1_lastrun.py",
...
def run(expInfo, thisExp, win, globalClock=None, thisSession=None):
    ...
    # --- Initialize components for Routine "Trial" ---
    answer_input = visual.TextBox2(
         win, text=None, placeholder=None, font='Arial',
         ori=0.0, pos=(0, 0), draggable=False, units='height',     letterHeight=0.1,
    ...
    prog = visual.Progress(
        ...
        barColor='white', backColor='white', borderColor='white', colorSpace='rgb',
    ...
    trial = data.Routine(
        name='trial',
        components=[key_resp_3, welcome_image],
    )
    ...
    for thisTrial in trials:
        ...
        Trial = data.Routine(
            name='Trial',
            components=[answer_input, polygon, stim_words, prog],
        )
        Trial.status = NOT_STARTED
        continueRoutine = True
        # update component parameters for each repeat
        ...
        points = 0  # Initialize points for this trial
        opacity = 1
        answering = False
        square_color = 'black'
        routine_time = question_time
        ...
        placeholder_text = word1 + "\n" + word2 + "\n" + word3 
        timer = core.CountdownTimer(question_time)
        stim_words.reset()
        # store start times for Trial
        Trial.tStartRefresh = win.getFutureFlipTime(clock=globalClock)
        Trial.tStart = globalClock.getTime(format='float')
        ...
        while continueRoutine:
            ...
            # *answer_input* updates
            
            # if answer_input is starting this frame...
            if answer_input.status == NOT_STARTED and tThisFlip >= 0-frameTolerance:
                # keep track of start time/frame for later
                answer_input.frameNStart = frameN  # exact frame index
                answer_input.tStart = t  # local t and not account for scr refresh
                answer_input.tStartRefresh = tThisFlipGlobal  # on global time
                win.timeOnFlip(answer_input, 'tStartRefresh')  # time at next scr refresh
                # add timestamp to datafile
                thisExp.timestampOnFlip(win, 'answer_input.started')
                # update status
                answer_input.status = STARTED
                answer_input.setAutoDraw(True)
            
            # if answer_input is active this frame...
            if answer_input.status == STARTED:
                # update params
                answer_input.setColor('white', colorSpace='rgb', log=False)
                answer_input.setSize([1,2], log=False)
            # Run 'Each Frame' code from code
            # Check for newline in answer input
            if '\n' in answer_input.text:
                continueRoutine = False  # Immediately end the routine
                print("Newline detected - ending routine early")
            
            # Get current time and calculate progress
            t = timer.getTime()
            progress = max(0, t / routine_time)  # 1.0 → 0.0
            
            # Set color based on current timer's thirds
            if progress < 1/3:  # Final third (red)
                bar_color = 'red'
            elif progress < 2/3:  # Middle third (yellow)
                bar_color = 'yellow'
            else:  # First third (green)
                bar_color = 'green'
            
            # Set width directly using progress (no max_bar_width needed)
            bar_width = progress  # Assuming your bar scales 0-1
            
            # Handle answer input
            if len(answer_input.text) >= 1:
                opacity = 0
                placeholder_text = ''
                print('set opacity', opacity)
                if not answering:
                    answering = True
                    timer = core.CountdownTimer(answer_time)
                    routine_time = answer_time
            
            # End routine if time expires
            if t <= 0:
                continueRoutine = False
            ...
            if polygon.status == STARTED:
                # update params
                polygon.setOpacity(opacity, log=False)
            ...
            if stim_words.status == NOT_STARTED and tThisFlip >= 0-frameTolerance:
                ...
                # add timestamp to datafile
                thisExp.timestampOnFlip(win, 'stim_words.started')
                # update status
            ...
            if stim_words.status == STARTED:
                # update params
                stim_words.setOpacity(opacity, log=False)
                stim_words.setSize([1,2], log=False)
                stim_words.setText(placeholder_text, log=False)
                stim_words.setPlaceholder(placeholder_text, log=False)
            ...
            if prog.status == STARTED:
                # update params
                prog.setColor(bar_color, colorSpace='rgb', log=False)
                prog.setFillColor('black', log=False)
                prog.setProgress(max(0, timer.getTime()) / routine_time, log=False)
        ...
        Trial.tStop = globalClock.getTime(format='float')
        Trial.tStopRefresh = tThisFlipGlobal
        trials.addData('answer_input.text',answer_input.text)
        # Run 'End Routine' code from code
        cleaned_answer = ''.join([c.lower() for c in answer_input.text if c.isalpha()])
        cleaned_solution = solution.lower().replace(" ", "")
        
        # Assign points (1 for correct, 0 for incorrect)
        if cleaned_answer == cleaned_solution:
            points = 1
        else:
            points = 0
        
        # Add to your data output
    ...
    Real_test = data.Routine(
        name='Real_test',
        components=[Real_test_message, real_test_notice, key_resp_6],
    )
    ...
    trials_2 = data.TrialHandler2(
        ...
        extraInfo=expInfo, 
        originPath=-1, 
        trialList=data.importConditions('CRA_conditions.csv'), 
        seed=None, 
    )
    ...
    for thisTrial_2 in trials_2:
        ...
        Trial.tStop = globalClock.getTime(format='float')
        Trial.tStopRefresh = tThisFlipGlobal
        trials_2.addData('answer_input.text',answer_input.text)
        # Run 'End Routine' code from code
        cleaned_answer = ''.join([c.lower() for c in answer_input.text if c.isalpha()])
        cleaned_solution = solution.lower().replace(" ", "")
